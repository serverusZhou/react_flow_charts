...
export default function() {
  const drawAPipe = drawAPipeWapper()
  return function (chartsInfo, cxt, config, lineConfig, items, choosenObj, hoverObj) {
    ...
    lineConfig.forEach(element => {
      if (element.type === 'arrow') { drawALine(chartsInfo, cxt, [element.from.position.x, element.from.position.y], [element.to.position.x, element.to.position.y]) }
      if (element.type === 'pipe') { drawAPipe(chartsInfo, cxt, [element.from.position.x, element.from.position.y], [element.to.position.x, element.to.position.y]) }
    })
  }
}
